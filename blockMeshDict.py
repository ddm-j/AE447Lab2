...
def gen_blockmeshdict(foil="0012", alpha_deg=4):
    ...
    """
    # Foil geometry
    c = 6*0.0254             # Geometric chord length
    alpha = np.deg2rad(alpha_deg)  # Angle of attack (in radians)
    NACA = [int(d) for d in foil]  # NACA 4-digit designation

    # Mesh dimensions
    scale = 1            # Scaling factor
    H = 50*c                # *Half* height of channel
    W = 2*c              # *Half* depth of foil (y-direction)
    D = 100*c               # Length of downstream section

    # Mesh resolution parameters
    Ni = 400             # Number of interpolation points along the foil
    Nx = 150             # Number of mesh cells along the foil
    ND = 150            # Number of cells in the downstream direction
    NT = 125             # Number of cells the transverse direction
    NW = 1               # Number of cells in the y-direction (along the foil axis)

    # Expansion rates
    ExpT = 5000           # Expansion rate in transverse direction
    ExpD = 500           # Expansion rate in the downstream direction
    ExpArc = 10          # Expansion rate along the inlet arc
    ...
    # Open file
    if os.path.exists("system/blockMeshDict"):
